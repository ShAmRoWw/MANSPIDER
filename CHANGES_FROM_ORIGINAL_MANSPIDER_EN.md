...
| State is created automatically in a per-scan folder | Each new automatic scan receives a timestamped, uniquely named subdirectory under `${XDG_STATE_HOME:-~/.local/state}/manspider/scans`, containing its SQLite database and sibling logs/reports/review marks. The parent directory can be overridden with `MANSPIDER_STATE_DIR`. | Users no longer need to choose a `--state-file` for every run, and files belonging to different scans do not mix. | Resume reuses the same folder. Legacy flat sessions remain discoverable without being moved; discovery inspects only the root and one child-directory level, without following symlinks. Explicit `--state-file` and `--resume` retain their exact paths. |
...
| Manual finding-review marks | Each finding can be marked reviewed and unmarked. A filter shows all, only unreviewed, or only reviewed findings; nothing is hidden by default. After saving a mark, an active review filter updates the list automatically, preserving the current results page and expanded file cards. Marks persist separately in `<session>.review`. | Specialists can remove already reviewed evidence from their working view without manual refresh, deleting findings, changing scan statistics, or contacting the scanned server again. |
...
| What changed | Brief explanation | Benefit | Example |
| --- | --- | --- | --- |
| File finding pages validate their result-set version | Replaced findings or externally changed review-filter membership prompt a restart of that file's pages while keeping already displayed evidence visible. Saving a review mark in the current filtered view automatically refreshes the list instead: file cards stay expanded, but their internal finding/evidence pages return to the preview. | A stale cursor cannot present an empty page as the end of the findings. | After retrying a file with 60 findings, restarting its pages exposes the remaining 10. The version marker is not an access key or authentication. |
